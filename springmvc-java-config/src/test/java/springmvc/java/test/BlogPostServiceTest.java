...
public class BlogPostServiceTest {
	...
	public void testBlogPostService() {
		...
		assertTrue(blogPostService.listAllBlogPostByUserAndTitleLike(user, "title 1").size() == 0);
		
		BlogPost blogPostDraft = new BlogPost();
		blogPostDraft.setTitle("title 2");
		blogPostDraft.setContent("content 2");
		blogPostDraft.setUser(user);
		
		blogPostService.saveAsDraft(blogPostDraft);
		assertTrue(blogPostService.listAllBlogPostByUserAndDraftStatus(user, true).size() > 0);
